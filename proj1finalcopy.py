# -*- coding: utf-8 -*-
"""proj1FinalCopy.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/165phQOaUWxBr97RHv498Y_ULSsU4GwKL
"""
...
def createLinear(nCount,whichTest):
    numSamples = nCount
    #training data has a margin between them of two
    if(whichTest=="training"):
    ...
    #test data will have a margin of 1 (purposely giving us more error here to be closer to natural world data)
    else:
      plusX = np.round(np.random.uniform(0, 5, numSamples), 2)
      plusY = np.round(np.random.uniform(0, 5, numSamples), 2)

      negX = np.round(np.random.uniform(6.2, 10, numSamples), 2)
      negY = np.round(np.random.uniform(6, 10, numSamples), 2)
    ...
    plt.title("Linearly separable Data for {} points ({})".format(numSamples, whichTest))
    plt.xlabel('X-axis')
    plt.ylabel('Y-axis')
...
def train_main(trainingInputDataPointsVector, yTrueClassVector):
    ...
    for setLimit in range(0, MAX_ITERATIONS):  # iterates starting from 0 to 1000
        ...
        for loopN in reversed(range(len(trainingInputDataPointsVector))):
...
def update_weight_vector(currentWeightsVector, dataPointsVector, in_yTrueClass):
    ...
    stepSize = 1  # maybe need to adjust this.
...
if __name__ == "__main__":
    ...
    data, yClass = createLinear(25,"training")
    # print(data)
    train_main(data, yClass)
    print("\n")  # separating the different stages of line finding.
    test_data, yClass = createLinear(30,"test")
